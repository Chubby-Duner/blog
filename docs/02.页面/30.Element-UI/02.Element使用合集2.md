...
![25](https://cdn.jsdelivr.net/gh/zchaoGe/image-hosting@master/element-ui/25.png)

**需求**假设我们从后端返回的数据，根据`某个字段`对每一行的数据`添加背景颜色`，便于用户区分筛选，这里我们用到el-table的`row-style`属性，参数如下；

![24](https://cdn.jsdelivr.net/gh/zchaoGe/image-hosting@master/element-ui/24.png)

::: warning 说明
行的style回调方法(Function({row, rowIndex})/Object),这里我们直接用row就够了，我们打印看一下，row就是当前行的数据
:::

![26](https://cdn.jsdelivr.net/gh/zchaoGe/image-hosting@master/element-ui/26.png)
...
![col](https://cdn.jsdelivr.net/gh/zchaoGe/image-hosting@master/element-ui/col.gif)
